...
// ---- COMMAND: LIST ----
  ...
  (async () => {
    ...
    try {
      ...
      const tasks = await pod.plan(requirement, contextBundle);
      console.log(`✅ Plan generated: ${tasks.length} tasks identified.\n`);
      ...
      for (const task of tasks) {
        ...
        if (finalCode) {
          finalChanges[task.file] = finalCode;
      ...
      if (Object.keys(finalChanges).length > 0) {
        ...
        for (const [file, code] of Object.entries(finalChanges)) {
          const targetPath = output || file;
          console.log(`   - ${file} -> ${targetPath}`);
          if (output || tasks.length === 1) {
             fs.writeFileSync(targetPath, code);
          } else {
             // Multi-file results without a single output flag
             // Future: ask to apply or write to multiple files
             console.log(`\n--- CONTENT FOR ${file} ---\n${code}\n`);
          }
        }
